...
**The Relative-Surface Rule.** Some surfaces here have no colour of their own worth speaking of — they are defined *entirely* by sitting a fixed distance from the canvas. The confidence ticker's band is the clearest case: `--landing-bg-2` plus a hairline border, and nothing else. **Move the canvas without moving them and they stop existing.** That happened on 2026-09-01: the canvas went 0.54 -> 0.52 for the AA fix, `--landing-bg-2` stayed at 0.47, and the band's luminance gap fell 0.0652 -> 0.0509 (contrast 1.442 -> 1.345). It was reported as the bar being *gone* — the 32 chips were rendering and animating the whole time. Fixed by applying the canvas's own -0.02 to it rather than re-solving: same delta, same colour space, relationship preserved. **When you change the canvas, grep for every token defined against it and move them all by the same amount.** Today that is `--landing-bg-2`; the `--background` / `--landing-bg` pair is the same rule already written down.
