...
function QualificationPage() {
  ...
  return (
    <Shell
      photo="qualification"
      title={
        current ? placesHeadline(current.qualLimit, current.isField, t) : t("qual.headlineFallback")
      }
    ...
    >
      {state.status === "loading" && (
        <PanelSkeleton title={t("qual.standingsSkeleton")} rows={10} />
      )}
        ...
        <>
          {tight.length > 0 && (
            <Panel
              ...
              subtitle={decided ? t("qual.tightSubtitleDecided") : t("qual.tightSubtitle")}
            >
      ...
      <p className="max-w-3xl text-[14px] leading-relaxed text-muted-foreground">
        ...
        {firstOut && (
          <>
            ...
            {t(
              Math.abs(firstOut.gap ?? 0) === 1 ? "qual.howToRead.point" : "qual.howToRead.points",
            )}
